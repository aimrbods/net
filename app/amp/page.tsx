...
export default async function AmpIndexPage() {
  const posts = (await getPosts()).slice(0, 12);
  ...
  const html = `<!doctype html><html amp lang="id"><head><meta charset="utf-8"><title>${escapeHTML(SITE.name)}</title><meta name="viewport" content="width=device-width,minimum-scale=1,initial-scale=1"><script async src="https://cdn.ampproject.org/v0.js"></script><style amp-custom>body{margin:0;background:#020617;color:#e5e7eb;font-family:Arial,sans-serif;padding:24px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:18px}.card{background:#0f172a;border:1px solid #1e293b;border-radius:20px;overflow:hidden;color:#fff;text-decoration:none}.card h3{padding:16px}</style></head><body><h1>${escapeHTML(SITE.name)}</h1><p>Tutorial SEO, AI, blogging, dan teknologi modern Indonesia</p><div class="grid">${cards}</div></body></html>`;
